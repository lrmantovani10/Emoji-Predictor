...
def process_input_gpt3(sentence, emotion):
    ...
    if emotion not in different_prompts and emotion != "standard":
        custom_prompt = "Complete in a " + emotion +" tone with an emoji at the end:"
    elif emotion == "standard":
        custom_prompt = "Complete the sentence:"
    else:
    ...
    response["choices"][0].text = response["choices"][0].text.rstrip().lstrip()
    messages_collection.insert_one({emotion:{"input":sentence, 
        "output":response}})
    return response["choices"][0].text.replace(sentence, "").rstrip().lstrip()
